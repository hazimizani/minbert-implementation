...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    # attention scores: QK^T / sqrt(d_k) -> [bs, num_attention_heads, seq_len, seq_len]
    attention_scores = torch.matmul(query, key.transpose(-1, -2))
    attention_scores = attention_scores / math.sqrt(self.attention_head_size)

    # apply attention mask (non-padding=0, padding=large negative)
    attention_scores = attention_scores + attention_mask

    # normalize the scores and apply dropout
    ...
    # concat multi-heads: [bs, num_heads, seq_len, head_size] -> [bs, seq_len, hidden_size]
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    # add-norm layer
    attn_output = self.add_norm(hidden_states, attn_output,
    ...
    output = self.add_norm(attn_output, ff_output,
